...
public class Rand6 implements ICommand, IGuildCommand {
    ...
    private void reroll(Member member, MessageReceivedEvent event) throws IOException {
        ...
        if ((date.getTime()) < Long.parseLong((rollMap.get(member) == null) ? (date.getTime()-1) + "" : rollMap.get(member))) {
            event.getChannel().sendMessage(new EmbedBuilder().setColor(Color.red).setDescription("Your reroll is still consumed.\nReset in `" + timeToReset(rollMap.get(member)) + "`.").build()).queue();
            return;
        } else {
            rollMap.put(member, (date.getTime() + 12*60*60*1000) + "");
            event.getChannel().sendMessage(new EmbedBuilder().setColor(Color.orange).setDescription(member.getAsMention() + " USED A REROLL. (Reset in `" + timeToReset(rollMap.get(member)) + "`)\n\nYour new operator, " + member.getAsMention() + ", is `" + getRandOp() + "`").build()).queue();
        }
        ...
        rollMap.keySet().stream().forEach(m -> {
            try {
                ...
                e.printStackTrace();
                event.getChannel().sendMessage(Messages.error().setDescription("AN ERROR OCCURED WHILE WRITING SAVE FILE...").build()).queue();
            }
        });
    ...
    private void resetReroll(Member member, MessageReceivedEvent event) throws IOException {
        ...
        if (rollMap.containsKey(member)) {
            ...
            event.getChannel().sendMessage(new EmbedBuilder().setColor(Color.cyan).setDescription("**Reseted reroll from " + member.getAsMention() + "**").build()).queue();
        } else
            event.getChannel().sendMessage(Messages.error().setDescription(member.getAsMention() + " has no active reroll timeouts.").build()).queue();
    ...
    public void action(String[] args, MessageReceivedEvent event) throws ParseException, IOException {
        ...
        if (!event.getMember().getVoiceState().inVoiceChannel()) {
            event.getChannel().sendMessage(Messages.error().setDescription("You need to be in a voice channel to use this command!").build()).queue();
            return;
        }
        ...
        switch (args[0]) {
            ...
            case "setops":
                if (args.length > 1) {
                    ...
                    event.getChannel().sendMessage(Messages.success().setDescription("Successfully set Ops source URL to `" + args[1] + "`.").build()).queue();
                } else {
                    event.getChannel().sendMessage(Messages.error().setDescription(help()).build()).queue();
                }
                return;
            ...
            case "rlist":
                if (saveFile.exists()) {
                    ...
                    if (saveFile.exists()) {
                        ...
                        event.getChannel().sendMessage(new EmbedBuilder().setDescription("**USED REROLLS:**\n\n" + outStr.toString()).build()).queue();
                    }
                }
                return;
            ...
            case "reset":
                ...
                if (PermissionHandler.check(Perm.MODERATOR, event)) return;
