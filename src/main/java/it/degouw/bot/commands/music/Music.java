package it.degouw.bot.commands.music;
...
import it.degouw.bot.audio.AudioInfo;
import it.degouw.bot.audio.TrackManager;
import it.degouw.bot.commands.ICommand;
import it.degouw.bot.commands.IGuildCommand;
import it.degouw.bot.audio.AudioPlayerSendHandler;
...
public class Music implements ICommand, IGuildCommand {
    ...
    private AudioEventListener audioEventListener = new AudioEventAdapter() {
        ...
        public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
            ...
            if (getTrackManager(guild).getQueuedTracks().size() < 2 && endlessMode) {
                ...
                if (guild.getTextChannelsByName(SSSS.getMusicChannel(guild), true).size() > 0)
                    guild.getTextChannelsByName(SSSS.getMusicChannel(guild), true).get(0).sendMessage(Messages.success().setDescription("Repeated queue. *(endless mode)*").build()).queue();
            }
    ...
    public void action(String[] args, MessageReceivedEvent event) throws ParseException, IOException {
        ...
        if (SSSS.getLockMusicChannel(guild) && !event.getTextChannel().getName().equals(SSSS.getMusicChannel(guild))) {
            Message msg = event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.red).setDescription(":warning:  " + event.getAuthor().getAsMention() + ", please only send music commands in the #" + STATIC.musicChannel + " channel!").build()).complete();
            new Timer().schedule(new TimerTask() {
                @Override
                public void run() {
                    msg.delete().queue();
        ...
        if (STATIC.music_volume > 0)
            getPlayer(guild).setVolume(STATIC.music_volume);
        ...
        switch (args.length) {
            ...
            case 2:
                switch (args[0].toLowerCase()) {
                    ...
                    case "channel":
                        if (PermissionHandler.check(Perm.ADMIN, event)) return;
                        if (args.length < 2) {
                        ...
                        event.getTextChannel().sendMessage(Messages.success().setDescription("Music channel successfully changed to `" + args[1].toLowerCase() + "`.").build()).queue();
                    ...
                    case "lockchannel":
                        ...
                        try {
                            ...
                            event.getTextChannel().sendMessage(Messages.success().setDescription("Music channel lock successfully set to `" + Boolean.parseBoolean(args[1]) + "`.").build()).queue();
                        } catch (Exception e) {
                            event.getTextChannel().sendMessage(Messages.error().setDescription(help()).build()).queue();
                        }
                        break;
                    ...
                    case "endless":
                        ...
                        event.getTextChannel().sendMessage(Messages.success().setDescription(":repeat:  Endless mode activated.").build()).queue();
